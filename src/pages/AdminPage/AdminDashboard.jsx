...
// ✅ UPDATED: Now targeting your newly decoupled HomeManagement subfolder structure
...
function AdminDashboard() {
  ...
  // Sync all buffers simultaneously from local Apache server disk
  useEffect(() => {
    ...
    .then(([headerData, mandateData, servicesData, directoryData, contactData, visionMissionData]) => {
      setHeaderState({
    ...
    <div className="max-w-6xl mx-auto my-12 px-4 select-none">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center border-b border-slate-200 pb-6 mb-8 gap-4">
        ...
        <div className="flex items-center gap-3">
          <span className="bg-emerald-50 text-emerald-700 text-[10px] font-black uppercase tracking-widest py-1.5 px-3 rounded-full border border-emerald-200 shadow-sm">
            ● Secure Link Synced
          </span>
          <button onClick={handleSystemLogout} className="bg-rose-50 text-rose-600 hover:bg-rose-100 hover:text-rose-700 transition-colors text-[10px] font-black uppercase tracking-widest py-1.5 px-4 rounded-xl border border-rose-200 shadow-sm cursor-pointer">
