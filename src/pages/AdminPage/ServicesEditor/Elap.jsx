...
function EmergencyLoanAssistance({ onBack, baseUrl, targetFile }) {
  const [formData, setFormData] = useState(null);
  const [loading, setLoading] = useState(true);
  const fileName = targetFile || 'services_elap.json';
  ...
  const handleSave = async () => {
    ...
    try {
      const response = await fetch(`${baseUrl}/api/content/save-config`, {
        ...
        // ✨ FIXED: Removed 'services/' path segment to match the destination schema rules
        body: JSON.stringify({ targetFile: fileName, data: formData })
      });
      if (response.ok) {
        toast.success("✨ ELAP trust fund configurations saved safely!", { id: loadingToast });
      } else {
      ...
      <div className="flex justify-between items-center border-b pb-4">
        <div>
          ...
          <h2 className="text-md font-black text-[#002B5B] uppercase tracking-wide mt-2 flex items-center gap-2">
            <span>{formData.iconHeader || '🛡️'}</span> {formData.title || 'ELAP Management'} Node
          </h2>
        </div>
