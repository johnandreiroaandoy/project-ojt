...
function CertificationOnSalaryReceived({ onBack, baseUrl, targetFile }) {
  const [fullPayload, setFullPayload] = useState(null); // Keeps the entire file intact
  const [loading, setLoading] = useState(true);
  const fileName = targetFile || 'certification_salary.json';

  // 🔄 1. Fetch the complete profile dataset from your backend server on mount
  useEffect(() => {
    let isMounted = true;
    const cacheBuster = `?v=${new Date().getTime()}`;
    
    // ✨ FIXED: Points directly to flat /data/ path instead of /data/services/
    fetch(`${baseUrl}/data/${fileName}${cacheBuster}`)
      ...
      .then(data => {
        if (isMounted) setFullPayload(data);
      })
      .catch(err => {
  ...
  // Shortcut variable pointing right into the targeted nested object branch
  const targetNode = fullPayload?.salaryCertification;

  // 📝 2. Field Change Mutation Handlers (scoped safely under salaryCertification)
  const handleRootFieldChange = (key, value) => {
    setFullPayload(prev => ({
      ...prev,
      salaryCertification: {
        ...prev.salaryCertification,
        [key]: value
      }
    }));
  };
  ...
  const handleDetailChange = (key, value) => {
    setFullPayload(prev => ({
      ...
      salaryCertification: {
        ...prev.salaryCertification,
        serviceDetails: { ...prev.salaryCertification.serviceDetails, [key]: value }
      }
    }));
  };

  const handleRequirementChange = (index, value) => {
    const updatedReqs = [...targetNode.requirements];
    updatedReqs[index] = value;
    setFullPayload(prev => ({
      ...prev,
      salaryCertification: { ...prev.salaryCertification, requirements: updatedReqs }
    }));
  };

  const handleStepChange = (index, key, value) => {
    const updatedSteps = [...targetNode.processingSteps];
    updatedSteps[index] = { ...updatedSteps[index], [key]: value };
    setFullPayload(prev => ({
      ...prev,
      salaryCertification: { ...prev.salaryCertification, processingSteps: updatedSteps }
    }));
  };

  // ➕/➖ 3. Dynamic Array Item Controllers
  const addRequirement = () => {
    setFullPayload(prev => ({
      ...prev,
      salaryCertification: {
        ...prev.salaryCertification,
        requirements: [...prev.salaryCertification.requirements, ""]
      }
    }));
  };
  ...
  const removeRequirement = (index) => {
    setFullPayload(prev => ({
      ...
      salaryCertification: {
        ...prev.salaryCertification,
        requirements: prev.salaryCertification.requirements.filter((_, i) => i !== index)
      }
    }));
  };

  const addStep = () => {
    const nextStepNum = targetNode.processingSteps.length + 1;
    const newStep = { step: nextStepNum, action: "", duration: "" };
    setFullPayload(prev => ({
      ...prev,
      salaryCertification: {
        ...prev.salaryCertification,
        processingSteps: [...prev.salaryCertification.processingSteps, newStep]
      }
    }));
  };

  const removeStep = (index) => {
    const filtered = targetNode.processingSteps.filter((_, i) => i !== index);
    const reIndexed = filtered.map((item, idx) => ({ ...item, step: idx + 1 }));
    // ✨ FIXED: Changed from 'setState' to 'setFullPayload' to resolve the crashing bug
    setFullPayload(prev => ({
      ...prev,
      salaryCertification: { ...prev.salaryCertification, processingSteps: reIndexed }
    }));
  };

  // 💾 4. Post integrated wrapper state safely back to storage
  const handleSave = async () => {
    const loadingToast = toast.loading(`💾 Pushing nested updates to backend ${fileName}...`);
    try {
      const response = await fetch(`${baseUrl}/api/content/save-config`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        // ✨ FIXED: Removed old 'services/' subdirectory layout prefix
        body: JSON.stringify({ targetFile: fileName, data: fullPayload })
      });
      if (response.ok) {
        toast.success("✨ Salary certification branch updated safely without affecting neighboring modules!", { id: loadingToast });
      } else {
  ...
  if (!fullPayload || !targetNode) return <div className="p-8 text-xs font-bold text-rose-500 text-center">❌ Failed to communicate or sync state architecture from file server.</div>;
    ...
    <div className="bg-white p-6 rounded-2xl border border-slate-200 space-y-6">
      ...
      <div className="flex justify-between items-center border-b pb-4">
        <div>
          ...
          <h2 className="text-md font-black text-[#002B5B] uppercase tracking-wide mt-2 flex items-center gap-2">
            <span>{targetNode.iconHeader || '📄'}</span> {targetNode.title || 'Salary Certification'} Manager
          </h2>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 bg-slate-50 p-4 rounded-xl border border-slate-200">
        <div>
          ...
          <input type="text" value={targetNode.title || ''} onChange={e => handleRootFieldChange('title', e.target.value)} className="w-full p-2 border rounded-lg text-xs font-bold bg-white mt-1 text-slate-800" />
        </div>
        <div className="md:col-span-2">
          ...
          <input type="text" value={targetNode.subtitle || ''} onChange={e => handleRootFieldChange('subtitle', e.target.value)} className="w-full p-2 border rounded-lg text-xs font-semibold bg-white mt-1 text-slate-600" />
        </div>
      </div>
      ...
      <div className="space-y-4 border border-slate-100 p-4 rounded-xl">
        ...
        <div>
          ...
          <input type="text" value={targetNode.serviceDetails?.name || ''} onChange={e => handleDetailChange('name', e.target.value)} className="w-full p-2 border rounded-lg text-xs font-bold mt-1 text-slate-800" />
        </div>

        <div>
          <label className="text-[9px] font-black text-slate-400 uppercase tracking-wider">System Purpose Context</label>
          <textarea rows={2} value={targetNode.serviceDetails?.purpose || ''} onChange={e => handleDetailChange('purpose', e.target.value)} className="w-full p-2 border rounded-lg text-xs text-slate-600 mt-1 resize-none leading-relaxed" />
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="text-[9px] font-black text-slate-400 uppercase tracking-wider">Responsible Office Branch</label>
            <input type="text" value={targetNode.serviceDetails?.office || ''} onChange={e => handleDetailChange('office', e.target.value)} className="w-full p-2 border rounded-lg text-xs text-slate-700 mt-1" />
          </div>
          <div>
            <label className="text-[9px] font-black text-slate-400 uppercase tracking-wider">Operational Complexity Classification</label>
            <input type="text" value={targetNode.serviceDetails?.classification || ''} onChange={e => handleDetailChange('classification', e.target.value)} className="w-full p-2 border rounded-lg text-xs text-slate-700 mt-1" />
          </div>
          <div>
            <label className="text-[9px] font-black text-slate-400 uppercase tracking-wider">Transaction Handling Category Mapping</label>
            <input type="text" value={targetNode.serviceDetails?.transactionType || ''} onChange={e => handleDetailChange('transactionType', e.target.value)} className="w-full p-2 border rounded-lg text-xs text-slate-700 mt-1" />
          </div>
          <div>
            <label className="text-[9px] font-black text-slate-400 uppercase tracking-wider">Authorized Application End Users</label>
            <input type="text" value={targetNode.serviceDetails?.eligibleUsers || ''} onChange={e => handleDetailChange('eligibleUsers', e.target.value)} className="w-full p-2 border rounded-lg text-xs text-slate-700 mt-1" />
          </div>
        </div>

        <div>
          <label className="text-[9px] font-black text-slate-400 uppercase tracking-wider">Aggregated Processing Duration Budget</label>
          <input type="text" value={targetNode.serviceDetails?.totalProcessingTime || ''} onChange={e => handleDetailChange('totalProcessingTime', e.target.value)} className="w-full p-2 border border-blue-200 bg-blue-50/40 text-[#002B5B] rounded-lg text-xs font-black mt-1 max-w-xs" />
        </div>
      </div>
      ...
      <div className="space-y-3 border border-slate-100 p-4 rounded-xl">
        ...
        <div className="space-y-2">
          {targetNode.requirements?.map((req, index) => (
      ...
      <div className="space-y-3 border border-slate-100 p-4 rounded-xl">
        ...
        <div className="space-y-3">
          {targetNode.processingSteps?.map((stepItem, index) => (
            <div key={`step-${index}`} className="grid grid-cols-12 gap-3 bg-slate-50 p-3 rounded-xl border border-slate-200 items-center relative">
              ...
              <div className="col-span-1 text-right">
                <button type="button" onClick={() => removeStep(index)} className="text-rose-500 hover:bg-rose-100 p-1.5 rounded-lg text-xs transition-colors">✕</button>
              </div>
            </div>
          ))}
        </div>
...
export default CertificationOnSalaryReceived;